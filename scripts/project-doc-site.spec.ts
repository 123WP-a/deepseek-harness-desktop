...
describe('publishableImage', () => {
  ...
  it('refuses a target whose real path escapes the repository', (ctx) => {
    ...
    try {
      ...
      // Windows denies unprivileged file-symlink creation (EPERM) unless
      // Developer Mode is on; the escape guard itself keeps being exercised
      // on Linux CI.
      ctx.skip()
    }
