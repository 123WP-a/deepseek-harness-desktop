...
The `desktop-events` plugin ([`src/desktop-events.ts`](src/desktop-events.ts)) watches the live session-event stream and forwards every `approval/asked` to the desktop shell's stdout event protocol (`dsh desktop-event:` lines → OS notifications); it never touches the decision chain. The `security-scan` tool ([`src/security-scan.ts`](src/security-scan.ts), core in [`src/security-scan-core.ts`](src/security-scan-core.ts)) registers `plugin_security_scan`: a static pre-install check over an unpacked plugin directory — executables, obfuscation density, credential-access combined with outbound network usage, shipped `.env`/key literals — whose high-risk verdict is mirrored onto the same channel. The `tasks-active` plugin ([`src/tasks-active.ts`](src/tasks-active.ts)) observes the job registry and mirrors "any running/stopping background job" onto a `tasks` desktop-event, which the desktop shell turns into its exit-confirmation guard.
...
- **Desktop notifications need the desktop shell** — `desktop-events` and high-risk `security-scan` verdicts always print their lines, but only the desktop shell renders them as OS notifications; a plain browser session sees the lines in server logs only.
- **Security scan v1 is local-directory only** — npm/git fetching and registry dependency auditing are deferred; the report lists dependency names without auditing them.
- **Tool-call audit is name-only and unrotated** — `tool-audit` appends tool name/status/timing lines to `$DSH_HOME/audit/tool-calls.jsonl` (never arguments or result content); the file grows without rotation.
- **Exit-guard task detection covers jobs only** — `tasks-active` watches background jobs; goals have no live server-side signal and are not part of the guard v1.
