...
contextBridge.exposeInMainWorld('desktopWindow', {
  ...
  /**
   * Tell the shell whether background tasks are still running, so closing
   * the window asks for confirmation instead of killing the server tree.
   * The page (e.g. a jobs/goal panel) clears it once work settles.
   * @param {boolean} active
   */
  setCloseGuard: (active) => ipcRenderer.send('dsh-desktop:set-close-guard', active === true),
})
