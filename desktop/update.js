...
const { recordStagedFingerprints, verifyStagedFingerprints } = require('./update-integrity.js')
...
async function installDshUpdate({ version, targetDir, cacheDir, registry = DEFAULT_REGISTRY }) {
  ...
  recordStagedFingerprints(targetDir, targetDir)
  fs.writeFileSync(
...
function activatePendingUpdate(pendingDir, activeDir) {
  ...
  const checked = verifyStagedFingerprints(pendingDir, pendingDir)
  if (!checked.ok) {
    throw new Error(`refusing to activate unsafe staged runtime: ${checked.reason}`)
  }
